<?php 
    ...
    if(isset($_GET['ubah'])){
        ...
        $defaultsatuan = $result['satuan_item'];
        $defaultkategori = $result['kategori_item'];
        $defaultsupplier = $result['kode_supplier_item'];
        $defaultharga_beli_terakhir_item = $result['harga_beli_terakhir_item'];
  ...
  <select id="Satuan" name="Satuan" value="<?php echo $satuan_item; ?>"required>
    <?php
        while ($result5 = mysqli_fetch_assoc($sql5)) {
            ...
            $selected = ($optionName == $defaultsatuan) ? 'selected' : '';
  ...
  <select id="kategori" name="kategori" required>
    <?php
      while ($result = mysqli_fetch_assoc($sql2)) {
        ...
        $selected = ($optionName == $defaultkategori) ? 'selected' : '';
  ...
  <select id="supplier" name="supplier" required>
    <?php
      while ($result = mysqli_fetch_assoc($sql3)) {
        ...
        $selected = ("$optionCode - $optionName" == $defaultsupplier) ? 'selected' : '';

        echo "<option value='$optionCode' $selected>$optionCode - $optionName</option>";
      }
    ?>
  </select>

  <label for="harga_beli_terakhir_item">Harga Beli Terakhir Item:</label>
  <input type="text" id="harga_beli_terakhir_item" name="harga_beli_terakhir_item" value= "<?php echo $harga_beli_terakhir_item; ?>"  required>
