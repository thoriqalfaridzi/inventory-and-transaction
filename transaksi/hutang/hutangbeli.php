...
$querytable = "SELECT DISTINCT nofaktur  , tanggal, jatuhtempo, metodebayarhutang, Supplier, status_bayar, jenis_bayar, hutang_terbayar, total_harga_beli_barang FROM transaksibeli WHERE jenis_bayar = 'Hutang' ORDER BY tanggal $orderClause;";
$sqltable = mysqli_query($conn, $querytable);

$querytable2 = "SELECT DISTINCT nofaktur  , tanggal, jatuhtempo, metodebayarhutang, Supplier, status_bayar, jenis_bayar, hutang_terbayar, total_harga_beli_barang FROM transaksibeli WHERE jenis_bayar = 'Hutang' ORDER BY tanggal $orderClause;";
               ...
               <?php 
                 while ($resulttable2 = mysqli_fetch_assoc($sqltable2)) {
                      if ($resulttable2['nofaktur'] !== $previousNofaktur2) {
                          ...
                          $querysum2 = "SELECT SUM(total_harga_beli_barang) AS total2 FROM transaksibeli WHERE nofaktur = '$previousNofaktur2'";
                      ...
                      ?>
                            <tr>
                                ...
                                <td class="thdead"><?php echo $resulttable2['Supplier']; ?></td>
...
<input type="text" id="searchInputsupplier" placeholder="Cari Berdasarkan Supplier..." />
<!-- <select id="searchInputstatus">
    ...
    while ($resulttable = mysqli_fetch_assoc($sqltable)) {
        ...
        <tr>
            ...
            <td style="text-align: center;" class="no-print">
                <a href="kelolahutang/kelolahutangbeli.php?ubah=<?php echo $resulttable['nofaktur']; ?>" type="button"
...
<footer class="footer">
  ...
  <script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        <?php
        $query = "SELECT * FROM transaksibeli WHERE jenis_bayar = 'Hutang'";
        ...
        while ($resulttable2 = mysqli_fetch_assoc($sqltable2)) {
            if ($resulttable2['nofaktur'] !== $previousNofaktur2) {
                if ($resulttable2['jenis_bayar'] == 'Hutang') {
                    $previousNofaktur2 = $resulttable2['nofaktur'];
                    // Tambahkan nilai total_harga_beli_barang ke totalhargafaktur
                    $querysum2 = "SELECT SUM(total_harga_beli_barang) AS total FROM transaksibeli WHERE nofaktur = '$previousNofaktur'";
                    $resultsum2 = mysqli_query($conn, $querysum2);
                    $rowsum2 = mysqli_fetch_assoc($resultsum2);
